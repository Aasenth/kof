const GIF = function() {
    ...
    function lzwDecode(minSize, data) {
        ...
        while (!done) { // JavaScript optimisers like a clear exit though I never use 'done' apart from fooling the optimiser
            ...
            if (code === clear) { // clear and reset the dictionary
                ...
                if (code >= dic.length) { dic.push(dic[last].concat(dic[last][0])) } else if (last !== clear) { dic.push(dic[last].concat(dic[code][0])) }
                d = dic[code];
                len = d.length;
    ...
    function parseImg() { // decodes image data to create the indexed pixel image
        ...
        if (gif.transparencyGiven) { frame.transparencyIndex = gif.transparencyIndex } else { frame.transparencyIndex = undefined }
    ...
    function processFrame(frame) { // creates a RGBA canvas image from the indexed pixel data.
        ...
        dat = cData.data;
        if (frame.interlaced) { pDat = deinterlaceBuf } else { pDat = pixelBuf }
    ...
    function parseExt() { // parse extended blocks
        ...
        if (blockID === GIF_FILE.GCExt) { parseGCExt() } else if (blockID === GIF_FILE.COMMENT) { gif.comment += st.readSubBlocks() } else if (blockID === GIF_FILE.APPExt) { parseAppExt() } else {
    ...
    function parseBlock() { // parsing the blocks
        ...
        } else if (blockId === GIF_FILE.EOF) { finnished(); return } else { parseExt() }
    ...
    function loadGif(filename) { // starts the load
        ...
        ajax.onload = function(e) {
            if (e.target.status === 404) { error("File not found") } else if (e.target.status >= 200 && e.target.status < 300) { dataLoaded(ajax.response) } else { error("Loading error : " + e.target.status) }
        };
    ...
    function togglePlay() {
        if (gif.paused || !gif.playing) { gif.play() } else { gif.pause() }
    }

    function seekFrame(frame) { // seeks to frame number.
        clearTimeout(timerID);
        gif.currentFrame = frame % gif.frames.length;
        if (gif.playing) { playing() } else { gif.image = gif.frames[gif.currentFrame].image }
    }
